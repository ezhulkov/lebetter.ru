...
(function($) {
        ...
        return this.each(function () {
            ...
            if ($.metadata) {
                ...
                if (container.metadata().content) {
                    container.attr("content", container.metadata().content);
                } //ajax
                if (container.metadata().aspectRatio) {
                    container.attr("aspectRatio", container.metadata().aspectRatio);
                } //ui.resize
                if (container.metadata().grid) {
                    container.attr("grid", container.metadata().grid);
                } //ui.grid
                if (container.metadata().gridx) {
                    container.attr("gridx", container.metadata().gridx);
                } //ui.grid
                if (container.metadata().gridy) {
                    container.attr("gridy", container.metadata().gridy);
                } //ui.grid
                if (container.metadata().handles) {
                    container.attr("handles", container.metadata().handles);
                } //ui.resize
            ...
            for (var i in btn) {
                if (btn[i] == "c") {
                    ...
                    $(this).find(".close:first").bind("click", function() {
                        if (!$.browser.msie) {
                            container.fadeOut(opt.effectDuration);
                        }
                        else {
                            container.hide();
                        }
                ...
                if (btn[i] == "i") {
                    ...
                    $(this).find(".iconizeContainer:first").bind("click", function() {
                        container.containerIconize(opt);
                    });
                }
            }
            var fadeOnClose = $.browser.mozilla || $.browser.safari;
            if (fadeOnClose) {
                $(this).find(".buttonBar:first img")
        ...
        this.each(function () {
            ...
            if ($(this).attr("collapsed") == "false") {
                ...
                if (opt.minimizeEffect == "fade") {
                    ...
                    container.animate({height:container.find(".n:first").outerHeight() + container.find(".s:first").outerHeight()}, opt.effectDuration, function() {
                ...
                if (opt.minimizeEffect == "fade") {
                    ...
                    container.find(".icon:first").hide();
                    container.animate({height:container.attr("h")}, opt.effectDuration, function() {
                        container.find(".icon:first").show();
                    });
                }
                if (container.hasClass("resizable")) {
    ...
    jQuery.fn.containerIconize = function (opt) {
        ...
        return this.each(function () {
            ...
            this.dockIcon = $("<img src='" + opt.elementsPath + "icons/" + (container.attr("icon") ? container.attr("icon") : "restore.png") + "' class='restoreContainer' width='32'/>").appendTo(dockPlace)
                ...
                if (container.hasClass("resizable") && container.attr("collapsed") == "false") {
                    container.resizable("enable");
                }
                ;
                $(this).remove();
    ...
    jQuery.fn.mb_fullscreen = function() {
        ...
        if (!$(this).is(".draggable") || $(this).is("[iconized='true']") || $(this).is("[collapsed='true']")) {
            return;
        }
